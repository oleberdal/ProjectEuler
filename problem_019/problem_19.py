...
def number_of_weekdays_on_x_of_month(x, weekday, from_date, to_date, initial_date):
    ...
    checked_below = not date1_before_date2(from_date, initial_date)
    checked_above = date1_before_date2(to_date, initial_date)

    number_of_weekdays = 0

    date = initial_date.copy() if not checked_below else next_date(initial_date, -1)
    ...
    while not checked_below or not checked_above:
        ...
        day += step
        if date[0] == x and day % 7 == weekday and not date1_before_date2(to_date, date) and not date1_before_date2(date, from_date):
            number_of_weekdays += 1
        if not checked_below and not date1_before_date2(from_date, date):
            checked_below = True
            date = next_date(initial_date, step)
            day = initial_day
            step = 1
        elif not checked_above and not date1_before_date2(date, to_date):
            checked_above = True
...
def next_date(date, step):
    if not 1 <= date[0] + step <= days_in_month(date[1], date[2]):
        if not 1 <= date[1] + step <= 12:
            date[2] += step
            date[1] -= step * 12
        date[1] += step
        date[0] -= step * days_in_month(date[1] - bool(step == 1), date[2])
    date[0] += step
...
def main():
    solution = number_of_weekdays_on_x_of_month(1, "Sunday", "01.01.1901", "31.12.2000", ("01.01.1900", "Monday"))
